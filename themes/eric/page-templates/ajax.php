...
switch($action) {
  ...
  case 'vote':
    $dataInsert = array (
        'semifinalist_id' => filter_input(INPUT_POST, 'sid', FILTER_SANITIZE_STRING),
        'semifinalist_name' => filter_input(INPUT_POST, 'sname', FILTER_SANITIZE_STRING),
        'ip_address' => addslashes($ipaddress),
        'voted_datetime' => $time_local
    );

    if($wpdb->insert( $wpdb->community_voting, $dataInsert)){
      $result['success'] = 1;
    } else {
      $result['success'] = 0;
    }
